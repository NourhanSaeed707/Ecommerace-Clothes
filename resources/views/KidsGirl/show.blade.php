     ...
     <br><br><br> <div class="container">
          <div class="row   justify-content-between">
              <div class="col-md-6 image_side">
                <?php
                  if($kidgirlCloth[0]['image'] != ""){
                    $pieces = explode(',',$kidgirlCloth[0]['image']);}
                ?>
                <img src="/images/kidsGirl/clothes/{{$pieces[0]}}" alt="cloth" width="100%" height="100%">
              </div> <!-- image_side -->
              <div class="col-md-5 info_side">
                <div class="item clothname">{{$kidgirlCloth[0]['name']}}</div><br>
                <div class="pricetotal">
                @if( $kidgirlCloth[0]['price']  != $kidgirlCloth[0]['total'] )
                <strike class="item clothprice"style="color:red">{{$kidgirlCloth[0]['price']}}$</strike>
                <div class="item clothtotal ">{{$kidgirlCloth[0]['total']}}$</div><br>
                @else
                <div class="item clothprice">{{$kidgirlCloth[0]['price']}}$</div><br><br>
                @endif
                </div>
              ...
              <div class="color">
                  ...
                  @php
                     $c = 0;
                  @endphp
                        @foreach(explode(',', $kidgirlCloth[0]['color']) as $info) 
                        @if($info != "")
                        @php $c++ @endphp
                        <br><a  href="/kidgirlclotheshowc/{{$info}}/{{$kidgirlCloth[0]['productid']}}">
                        @if($c==1)
