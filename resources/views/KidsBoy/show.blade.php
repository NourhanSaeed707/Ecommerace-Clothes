     ...
     <br><br><br> <div class="container">
          <div class="row   justify-content-between">
              <div class="col-md-6 image_side">
                <?php
                  if($kidboyCloth[0]['image'] != ""){
                    $pieces = explode(',',$kidboyCloth[0]['image']);}
                ?>
                <img src="/images/kidsBoy/clothes/{{$pieces[0]}}" alt="cloth" width="100%" height="100%">
              </div> <!-- image_side -->
              <div class="col-md-5 info_side">
                <div class="item clothname">{{$kidboyCloth[0]['name']}}</div><br>
                <div class="pricetotal">
                @if( $kidboyCloth[0]['price']  != $kidboyCloth[0]['total'] )
                <strike class="item clothprice"style="color:red">{{$kidboyCloth[0]['price']}}$</strike>
                <div class="item clothtotal ">{{$kidboyCloth[0]['total']}}$</div><br>
                @else
                <div class="item clothprice">{{$kidboyCloth[0]['price']}}$</div><br><br>
                @endif
                </div>
              ...
              <div class="color">
                  ...
                  @php
                     $c = 0;
                  @endphp
                        @foreach(explode(',', $kidboyCloth[0]['color']) as $info) 
                        @if($info != "")
                        @php $c++ @endphp
                        <br><a  href="/kidboyclotheshowc/{{$info}}/{{$kidboyCloth[0]['productid']}}">
                        @if($c==1)
                          @php $color=$info @endphp
                           <br> <input type="image" src="/images/kidsBoy/color/{{$info}}" alt="color" width="42px" height="42px" class="colorWithBorder"><br><br>
                        @else
                        <br> <input type="image" src="/images/kidsBoy/color/{{$info}}" alt="color" width="42px" height="42px" class="colorWithoutBorder"><br><br>
                        @endif
                        </a> 
                        @endif
                        @endforeach
